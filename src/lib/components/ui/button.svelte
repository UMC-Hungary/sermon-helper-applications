<script lang="ts">
	...
	export let buttonVariant: ButtonProps["variant"] = "default";
	export let buttonSize: ButtonProps["size"] = "default";
	...
	$: variantClass = variantClasses[buttonVariant || "default"];
	$: sizeClass = sizeClasses[buttonSize || "default"];
	$: computedClass = cn(baseClasses, variantClass, sizeClass, className);
</script>

{#if href}
	<a {href} data-slot="button" class={computedClass} role="button" tabindex="0" {...$$restProps}>
		<slot />
	</a>
{:else}
	<button {type} {disabled} data-slot="button" class={computedClass} {...$$restProps}>
		<slot />
	</button>
{/if}